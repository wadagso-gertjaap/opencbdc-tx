    ...
    class http_server : public server_interface {
      ...
      private:
        ...
        handle_error(Json::Value params,
                         const server_type::result_callback_type& callback, int code, std::string message)
            -> bool;

        auto handle_number(Json::Value params,
                         const server_type::result_callback_type& callback, uint64_t number)
            -> bool;

        auto handle_boolean(Json::Value params,
                         const server_type::result_callback_type& callback, bool result)
            -> bool;
