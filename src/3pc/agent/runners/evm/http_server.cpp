...
namespace cbdc::threepc::agent::rpc {
    ...
    auto http_server::request_handler(
        ...
        } else if(method == "eth_decodeRawTransaction") {
        ...
        } else if(method == "eth_getCompilers"
    ...
    auto http_server::handle_get_storage_at(
        ...
        auto runner_params
            = cbdc::make_buffer(storage_key{std::move(maybe_addr.value()),
    ...
    auto http_server::parse_evm_log_query(
        ...
        if(params[0]["address"].isString()) {
            ...
            if(maybe_addr) {
                qry.m_addresses.push_back(std::move(maybe_addr.value()));
            } else {
                parseError = true;
            }
        ...
        if(params[0]["topics"].isArray()) {
            for(auto& val : params[0]["topics"]) {
                ...
                if(maybe_topic) {
                    qry.m_topics.push_back(std::move(maybe_topic.value()));
                } else {
                    parseError = true;
                }
            }
        ...
        if(block_count < 0) {
            ret["error"] = Json::Value();
            ret["error"]["code"] = -32204;
            ret["error"]["message"] = "From block cannot be after to block";
            callback(ret);
            return std::nullopt;
        }

        if(block_count * qry.m_addresses.size() > 100) {
            ret["error"] = Json::Value();
            ret["error"]["code"] = -32205;
    ...
    auto http_server::handle_get_logs(
        ...
        return exec_tx(callback,
                       ...
                       runner_params,
                       true,
                       [callback, runner_params, qry, this](
                           ...
                           auto maybe_logs
                               = cbdc::from_buffer<std::vector<evm_log_index>>(
                                   it->second);
                           ...
                           for(auto& log_idx : logs) {
                               for(auto& log : log_idx.m_logs) {
                                   ...
                                   for(auto& have_topic : log.m_topics) {
                                       for(auto& want_topic : qry.m_topics) {
                                   ...
                                   if(match) {
                                       ret["result"].append(tx_log_to_json(
                                           log,
    ...
    auto http_server::handle_get_block(
        ...
        return fetch_block(
            ...
            [this, callback, include_tx_details](
                ...
                ret["result"]["timestamp"] = to_hex_trimmed(evmc::uint256be(timestamp));
