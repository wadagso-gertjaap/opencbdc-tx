...
namespace cbdc::threepc::broker {
    ...
    auto impl::begin(begin_callback_type result_callback) -> bool {
        if(!m_ticketer->get_ticket_number(
               [this, result_callback](
                   std::optional<threepc::ticket_machine::interface::
    ...
    void impl::handle_ticket_number(
        ...
        std::optional<
            threepc::ticket_machine::interface::get_ticket_number_return_type>
            res) {
        if(!res.has_value()) {
            result_callback(error_code::ticket_number_assignment);
            return;
        }
        std::visit(overloaded{[&](const threepc::ticket_machine::interface::
                              ...
                              [&](const threepc::ticket_machine::interface::
                                      error_code& /* e */) {
                                  result_callback(
    ...
    void impl::handle_lock(
        ...
        const threepc::runtime_locking_shard::interface::try_lock_return_type&
            res) {
        auto result = std::visit(
            overloaded{
                [&](threepc::runtime_locking_shard::value_type v)
                ...
                [&, key](threepc::runtime_locking_shard::shard_error e)
                    -> try_lock_return_type {
    ...
    auto impl::try_lock(ticket_number_type ticket_number,
        ...
        auto maybe_error = [&]() -> std::optional<error_code> {
            ...
            if(!m_directory->key_location(
                   key,
                   [=](std::optional<
                       threepc::directory::interface::key_location_return_type>
                           res) {
    ...
    void impl::handle_prepare(
        ...
        ticket_number_type ticket_number,
        uint64_t shard_idx,
        threepc::runtime_locking_shard::interface::prepare_return_type res) {
    ...
    auto impl::do_handle_prepare(
        ...
        uint64_t shard_idx,
        const threepc::runtime_locking_shard::interface::prepare_return_type&
    ...
    auto impl::do_commit(const commit_callback_type& commit_cb,
        ...
        for(auto& shard : ts->m_shard_states) {
            ...
            if(!m_shards[sidx]->commit(
                   ticket_number,
                   [=](const threepc::runtime_locking_shard::interface::
    ...
    void impl::handle_commit(
        ...
        ticket_number_type ticket_number,
        uint64_t shard_idx,
        threepc::runtime_locking_shard::interface::commit_return_type res) {
        auto callback = false;
        auto maybe_error = [&]() -> std::optional<error_code> {
        ...
        for(auto& shard : t_state->m_shard_states) {
            ...
            if(!m_shards[shard.first]->prepare(
                   ...
                   [this,
                    ...
                    ticket_number,
                    shard_idx
                    = shard.first](const threepc::runtime_locking_shard::
    ...
    void impl::handle_rollback(
        ...
        ticket_number_type ticket_number,
        uint64_t shard_idx,
        threepc::runtime_locking_shard::interface::rollback_return_type res) {
        auto callback = false;
        auto maybe_error = [&]() -> std::optional<error_code> {
            std::unique_lock lll(m_mut);
            auto ittt = m_tickets.find(ticket_number);
    ...
    void impl::handle_find_key(
        ...
        std::optional<threepc::directory::interface::key_location_return_type>
            res) {
    ...
    void impl::handle_finish(
        ...
        ticket_number_type ticket_number,
        uint64_t shard_idx,
        threepc::runtime_locking_shard::interface::finish_return_type res) {
        auto callback = false;
        auto maybe_error = [&]() -> std::optional<error_code> {
            std::unique_lock lll(m_mut);
            auto ittt = m_tickets.find(ticket_number);
